...
- **REVIEWER_DIFFICULTY = nightmare** — Controls how adversarial the reviewer is. Three levels:
  - `medium`: MCP-based review, the executor controls what context the reviewer sees.
  ...
  - `nightmare` (default): Everything in `hard` + **Codex exec reviewer reads the repo directly** via `codex exec` (the executor cannot filter what the reviewer sees) + **Adversarial Verification** (the reviewer independently checks if code matches claims).
